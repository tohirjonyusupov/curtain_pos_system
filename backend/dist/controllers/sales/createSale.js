...
async function generateReceiptNo(tx, storeId) {
    const now = new Date();
...
async function createSale(req, res) {
    try {
        ...
        if (Number.isNaN(discount) || discount < 0)
            return res.status(400).json({ error: "discount must be >= 0." });
        ...
        const normalizedItems = items.map((it) => {
            ...
            if (Number.isNaN(unitPrice) || unitPrice < 0)
                throw new Error("Invalid unitPrice in items.");
            return {
                productId,
                qty: round3(qty),
        ...
        const result = await prisma_1.prisma.$transaction(async (tx) => {
            ...
            if (!cashier)
                throw new Error("Cashier not found for this store.");
            ...
            for (const it of normalizedItems) {
                ...
                if (!inv)
                    throw new Error(`Inventory not found for productId=${it.productId}`);
            ...
            if (total < 0)
                throw new Error("total cannot be negative (discount too large).");
            const paidAmount = paidAmountRaw !== null
                ? paidAmountRaw
                : paymentType === "credit"
                    ? 0
                    : total;
            if (paidAmount > total)
                throw new Error("paidAmount cannot be greater than total.");
            const receiptNo = await generateReceiptNo(tx, storeId);
        ...
        if (msg.includes("Ombor yetarli emas") ||
